...
function BottomNav({ active, go }: { active: View; go: (view: View) => void }) {
  ...
  return <nav className="tab-bar" aria-label="Primary navigation">{items.map((item) => <button key={item.label} onClick={() => go(item.view)} className={item.view === active ? "active" : ""}>{item.label}</button>)}</nav>;
}
