...
function BottomNav({ active, go }: { active: View; go: (view: View) => void }) {
  ...
  return <nav className="tab-bar" aria-label="Primary navigation"><div className="desktop-brand" aria-label="FretLab"><span>F</span><div><strong>FretLab</strong><small>Practice one key.<br/>Know the whole neck.</small></div></div><div className="nav-items">{items.map((item,index) => <button key={item.label} onClick={() => go(item.view)} className={item.view === active ? "active" : ""}><i/>{item.label}<small>0{index+1}</small></button>)}</div><div className="desktop-side-progress"><div><span>Weekly goal</span><strong>68 / 90 min</strong></div><i><b/></i><small>Three sessions this week</small></div><p className="desktop-nav-note">One connected practice system for the entire fretboard.</p></nav>;
}
...
function DesktopPracticeStudio({ go, sessionKey }: { go: (view: View) => void; sessionKey: KeyName }) {
  ...
  return <section className="desktop-studio" aria-label="Practice studio"><div className="studio-board"><div className="studio-heading"><div><p className="kicker">Practice studio</p><h2>See one key across the neck</h2></div><div className="studio-modes" role="group" aria-label="Fretboard labels">{(["Notes","Degrees","Roots"] as const).map((item) => <button className={mode === item ? "active" : ""} onClick={() => setMode(item)} key={item}>{item}</button>)}</div></div><Fretboard notes={notes} low={0} high={12} labelMode={mode === "Degrees" ? "degree" : "note"} rootKey={sessionKey}/><div className="studio-legend"><span><i className="root-dot"/>Root</span><span><i/>Scale tone</span><span>Tap Train to test recall without labels.</span></div></div><aside className="tempo-widget"><div className="tempo-orbit"><div className={`beat-orbit ${running ? "running" : ""}`}>{[0,1,2,3].map((beat) => <i style={{animationDelay:`${beat*(60/bpm)}s`,animationDuration:`${60/bpm}s`}} key={beat}/>)}</div><strong>{bpm}</strong><span>bpm</span></div><div className="tempo-controls"><button onClick={() => setBpm((value) => Math.max(40,value-4))} aria-label="Decrease tempo">−</button><button className="tempo-play" onClick={() => setRunning(!running)}>{running ? "Pause" : "Start"}</button><button onClick={() => setBpm((value) => Math.min(180,value+4))} aria-label="Increase tempo">+</button></div><button className="studio-train" onClick={() => go("train")}>Start recall drill <span>→</span></button></aside></section>;
}
