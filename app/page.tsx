...
function Ring({ value, size = 46, label }: { value: number; size?: number; label?: string }) {
...
function Fretboard({ notes, low, high, mini = false, interactive = false, found = new Set<string>(), onCell, labelMode = "note", rootKey = "G" }: { notes: Note[]; low: number; high: number; mini?: boolean; interactive?: boolean; found?: Set<string>; onCell?: (s: number, f: number) => void; labelMode?: "note" | "degree"; rootKey?: KeyName }) {
  ...
  return <div className={`fretboard ${mini ? "mini" : ""}`} aria-label={`Guitar fretboard, frets ${low} to ${high}`}>
    <div className="fret-grid" style={{ gridTemplateColumns: `repeat(${frets.length}, minmax(0, 1fr))` }}>
      {[6,5,4,3,2,1].flatMap((s) => frets.map((f) => {
        const id = `${s}:${f}`; const hasNote = noteMap.has(id); const isFound = found.has(id); const pc = (OPEN_PC[s] + f) % 12; const degree = (pc - keyPc(rootKey) + 12) % 12;
        const label = labelMode === "degree" ? intervals[degree] : noteNames[pc];
        return interactive ? <button key={id} className={`fret-cell ${hasNote ? "target" : ""} ${isFound ? "found" : ""}`} onClick={() => onCell?.(s,f)} aria-label={`${STRING_NAMES[s]} string, fret ${f}${hasNote ? `, ${label}` : ""}`}>{hasNote && <span>{mini ? "" : label}</span>}</button> : <div key={id} className={`fret-cell ${hasNote ? "target found" : ""}`}>{hasNote && <span>{mini ? "" : label}</span>}</div>;
      }))}
    </div>
    {!mini && <div className="fret-labels" style={{ gridTemplateColumns: `repeat(${frets.length}, minmax(0, 1fr))` }}>{frets.map((f) => <span key={f}>{f === 0 ? "open" : f}</span>)}</div>}
  </div>;
}
...
function BottomNav({ active, go }: { active: View; go: (view: View) => void }) {
  const items: { label: string; view: View; icon: string }[] = [{ label:"Today",view:"today",icon:"◒" },{ label:"Drills",view:"drills",icon:"≡" },{ label:"Train",view:"train",icon:"◎" },{ label:"Keys",view:"keys",icon:"◌" },{ label:"You",view:"progress",icon:"↗" }];
  return <nav className="tab-bar" aria-label="Primary navigation">{items.map((item) => <button key={item.label} onClick={() => go(item.view)} className={item.view === active ? "active" : ""}><span>{item.icon}</span>{item.label}</button>)}</nav>;
}
...
function Train({ selectedKey, setSelectedKey }: { selectedKey: KeyName; setSelectedKey: (key: KeyName) => void }) {
  ...
  useEffect(() => setFound(new Set()),[selectedKey]);
...
function KeyDetail({ go, selectedKey }: { go: (view: View) => void; selectedKey: KeyName }) {
  ...
  return <div className="screen-content detail-screen"><AppHeader title={`${selectedKey} major`} meta="Key" onBack={() => go("keys")} action={<span className="key-badge">{selectedKey}</span>}/><p className="detail-lede">{selectedKey === "G" ? "One sharp" : `${scale.filter((n) => n.includes("#") || n.includes("b")).length} accidentals`} · relative minor {scale[5]}</p><SegmentTabs labels={["Scales","Chords","Theory"]} active={tab} onChange={setTab}/>{tab === "Scales" && <><section className="section"><div className="section-head"><h2>The seven notes</h2></div><div className="degree-strip">{scale.map((note,i) => <div className={i === 0 || i === 4 ? "strong" : ""} key={note}><strong>{note}</strong><span>{i+1}</span><small>{roles[i]}</small></div>)}</div></section><section className="section shape-section"><div className="section-head"><h2>Five shapes, one scale</h2><button onClick={() => go("scale-detail")}>Open shape →</button></div><div className="shape-row">{[[0,3],[2,5],[4,7],[6,9],[8,12]].map(([low,high],i) => <button className={i === 1 ? "active" : ""} onClick={() => go("scale-detail")} key={i}><span>{["I","II","III","IV","V"][i]}</span><Fretboard notes={scaleShape(selectedKey,low,high)} low={low} high={high} mini rootKey={selectedKey}/></button>)}</div><p>Those five shapes are the same seven notes seen from five places on the neck. Learn where they overlap and the neck stops being twelve separate frets.</p></section></>}{tab === "Chords" && <article className="tab-copy"><h2>Chords in {selectedKey}</h2><p>{scale.map((note,i) => `${note}${["","m","m","","","m","dim"][i]}`).join(" · ")}</p></article>}{tab === "Theory" && <article className="tab-copy"><h2>What makes it major</h2><p>The distance pattern is whole, whole, half, whole, whole, whole, half. Keep those distances and the major sound survives in every key.</p></article>}</div>;
}
...
export default function FretLabPage() {
  ...
  return <main className="site-shell" style={vars}>
    <header className="web-header"><button className="wordmark" onClick={() => go("today")} aria-label="FretLab home"><span>F</span><b>FretLab</b></button><p>Practice one key. Know the whole neck.</p><div className="header-meta"><span className="live-dot"/>31 day streak</div></header>
    ...
    {!(["runner","summary","drill-detail","scale-detail","routine-detail","guided"] as View[]).includes(view) && <BottomNav active={primaryView} go={go}/>} 
  </main>;
}
